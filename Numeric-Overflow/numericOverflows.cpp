...
/// <summary>
/// Template function that returns a string representation of the result
/// </summary>
/// <typeparam name="T">A type that with basic math functions</typeparam>
/// <param name="result">final result of the math caluluations</param>
/// <returns>string representation of the result, EOVERFLOW, or EUNDERFLOW</returns>
template <typename T>
std::string get_result_string(T result)
{
    if (result == 0)
    {
        if (errno == EOVERFLOW)
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        // Test the suspect addition operation to ensure that no overflow occurs regardless of the representation used 
...
void test_overflow()
{
    ...
    std::cout << get_result_string(+result) << std::endl;
